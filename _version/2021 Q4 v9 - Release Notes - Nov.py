...
# MAGIC       - Functionality to choose one or more algorithms to run simultaneously.
# MAGIC       - Functionality to control model iterations for selected algorithms. 
# MAGIC       - Functionality to perform EDA and choose the final dataframe for model training.  
...
# MAGIC         - Training models for the following target definitions:
# MAGIC           - Binary Classification (Catboost/LightGBM/XGBoost)
# MAGIC           - Regression (LightGBM/XGBoost)
# MAGIC         - Functionality to choose one or more algorithms to run simultaneously.
# MAGIC         - Functionality to control model iterations for selected algorithms.  
...
# MAGIC |dnb_risk_scores_chg_us|	RISK|	US| .| 	US Standard Risk Scores change indicators built over 1m, 3m, 6m, 9m, 12m, 24m, 36m, 48m, 60m monthly changes. |
# MAGIC |dnb_marketing_scores_chg_us	| MKT|	US|	.| 	US Standard Marketing Scores change indicators built over 1m, 3m, 6m, 9m, 12m, 24m, 36m, 48m, 60m monthly changes.|
# MAGIC |dnb_id_geo_chg_us|	RISK, MKT|	US|	.|	D&B Core Business level attributes change indicators built over 1m, 3m, 6m, 9m, 12m, 24m, 36m, 48m, 60m monthly changes for US Businesses. Includes identification, geography, status and other key business operating indicators.|
# MAGIC |dnb_bus_firm_site_chg_us| RISK, MKT|	US|	.|	Business Firmographics change indicators built over 1m, 3m, 6m, 9m, 12m, 24m, 36m, 48m, 60m monthly change for US sites. DUNS Firmographics data for all Site Locations.|
# MAGIC |dnb_bus_firm_gsem_chg_us|	RISK, MKT|	US|	.|	Business Firmographics change indicators built over 1m, 3m, 6m, 9m, 12m, 24m, 36m, 48m, 60m monthly changes for US HQ. This view is typically used for Risk processing for HQ level modeling purposes. |
# MAGIC |dnb_risk_scores_chg_intl|	RISK|	INTL|	.|	International Standard Risk Scores change indicators built over 1m, 3m, 6m, 9m, 12m, 24m, 36m, 48m, 60m monthly changes.|
# MAGIC |dnb_marketing_scores_chg_intl|	MKT|  INTL|	.|	International Standard Marketing Scores change indicators built over 1m, 3m, 6m, 9m, 12m, 24m, 36m, 48m, 60m monthly changes.|
# MAGIC |dnb_id_geo_chg_intl|	RISK, MKT|	INTL|	.|	D&B Core Business level attributes change indicators built over 1m, 3m, 6m, 9m, 12m, 24m, 36m, 48m, 60m monthly changes for International Businesses. Includes identification, geography, status and other key business operating indicators.|
# MAGIC |dnb_bus_firm_site_chg_intl|	RISK, MKT|	INTL|	.|Business Firmographics change indicators built over 1m, 3m, 6m, 9m, 12m, 24m, 36m, 48m, 60m monthly change for International sites. DUNS Firmographics data for all Site Locations.|
# MAGIC |dnb_bus_firm_gsem_chg_intl|	RISK, MKT|	INTL|	.|	Business Firmographics change indicators built over 1m, 3m, 6m, 9m, 12m, 24m, 36m, 48m, 60m monthly changes for International HQ. This view is typically used for Risk processing for HQ level modeling purposes.| 
# MAGIC |dnb_esg|	RISK, MKT|	US, INTL|	Pilot|	Environmental, Social, and Governance ratings for US businesses. |
# MAGIC |dnb_diversity_ethnicity_data|	RISK|	Global|	.|	Indicators if a business is owned by individuals belonging to specific ethnicities.| 
# MAGIC |dnb_diversity_socio_economic_data|	RISK|	Global|	.|	Indicators that describe the socio-economic classification of the business owner.| 
...
# MAGIC |blackknight_assessment_matched| Assessor â€“ Collection of property data from 3,142 US County Assessors with insight on property value, owner, owner address for property taxes, etc.|
# MAGIC |blackknight_assignment_matched| Information on loan assignment transfers from current owner to buyer.|
# MAGIC |blackknight_deed_matched|	County Recorder-supplied Deed records from all Countries in the US. Contains info on property, buyer, seller, sales price, tax info, etc.|
# MAGIC |blackknight_nod_matched|	Notice of Default â€“ Information on properties in stages of pre-foreclosure / notice of default.|
# MAGIC |blackknight_release_matched|	Instruments which release a property mortgage after events.|
# MAGIC |blackknight_sitexcre_matched|	Enhanced CRE File â€“ A comprehensive view of commercial properties across the U.S. with SiteX CRE Enhanced Data, multiple public and private data sources are merged, distilled and analyzed, and the resulting insights are then added to data at the property level. This enhanced data product is designed to supplement existing data and reports by providing unique indicators, estimates, risk measures and contextual information.|
# MAGIC |blackknight_assessment_land_use|	Contains code descriptions for Standardized Land Use Code in Assessment File; Also provides RESFLAG which can be used to quickly identify / include / exclude Residential or Commercial properties; Indicator is sourced from Partner mapping.|
# MAGIC |blackknight_code_fips|	Provides mapping from five-digit FIPS Code in in all layouts to corresponding County Name and State Name.|
# MAGIC |blackknight_code_description|	Contains descriptions for codes present in Black Knight data; Data Type denotes in which file layout the code can be found.|
# MAGIC |blackknight_sam_matched|	Stand-Alone Mortgage â€“ County Recorder-supplied mortgage transaction data; captured separately from Deed information.|
...
# MAGIC |smartinsider_quantitative_data| Smart Insider analyses share transactions made by directors and senior employees in the shares of their own companies.|
