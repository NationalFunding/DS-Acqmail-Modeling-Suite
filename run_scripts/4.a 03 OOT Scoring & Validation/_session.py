...
class NatFunSession:
    ...
    def get_response_class_score_table(self, load_date, class_):
        """
        Given a load_date (either 'YYYY-MM-DD' string or datetime/date),
        return the latest scored_{class_}_response_YYYYMM table
        whose YYYYMM suffix <= the load_date's YYYYMM.
        """

        # Normalize load_date -> load_yyyymm (int)
        if isinstance(load_date, (datetime, date)):
            load_yyyymm = int(load_date.strftime("%Y%m"))
        else:
            # assume string like '2025-06-01'
            load_yyyymm = int(datetime.strptime(load_date, "%Y-%m-%d").strftime("%Y%m"))

        tables = spark.sql("SHOW TABLES IN nf_workarea").toPandas()
        pattern = rf"^scored_{class_}_response_\d{{6}}$"

        custom_score_tables = tables[
            tables["tableName"].str.contains(pattern, regex=True)
        ]

        # extract YYYYMM suffix as int
        custom_score_tables = custom_score_tables.copy()
        custom_score_tables["yyyymm"] = (
            custom_score_tables["tableName"]
            .str.extract(r"(\d{6})$", expand=False)
            .astype(int)
        )

        # keep only tables with yyyymm <= load_yyyymm
        valid = custom_score_tables[custom_score_tables["yyyymm"] <= load_yyyymm]

        if valid.empty:
            raise ValueError(
                f"No scored tables found with date <= load_date ({load_date}) for class {class_}"
            )

        # pick the row with the max yyyymm
        best_row = valid.loc[valid["yyyymm"].idxmax()]
        response_c_score_table = best_row["tableName"]

        return response_c_score_table
    ...
    def _initialize_date_information(self):
        ...
        self.resp_c1_score_table = self.get_response_class_score_table(self.load_dt, "c1")
        self.resp_c2_score_table = self.get_response_class_score_table(self.load_dt, "c2")
    ...
    def query_model_data(self, model_version):
        ...
        if model_version == 'V6':
            ...
            lmon = f"{self.load_dt.month:02d}",
            lyr_prior = self.load_dt_prior.year,
            lmon_prior = f"{self.load_dt_prior.month:02d}",
            lyr_qtr = self.lyr_qtr,
            lmon_qtr = self.lmon_qtr,
            db = self.db,
            run_date = self.run_dt.strftime('%Y%m%d'),
            resp_c1_score_table = self.resp_c1_score_table,
            resp_c2_score_table = self.resp_c2_score_table,
            version = 'V7',
            mth_title = self.mth_title
    ...
    def conduct_brand_specific_feature_engineering(self, model_data):
        ...
        pd_brand['months_since_first'] = np.where(pd_brand['months_since_first'] > 0, pd_brand['months_since_first'], -999)
        pd_brand['months_since_last'] = np.where(pd_brand['months_since_last'] > 0, pd_brand['months_since_last'], -999)

        for i in range(0,6):
            pd_brand[f'cadence_m{i+1}'] = np.where(pd_brand[f'{brand}_cadence_pattern'].str[i]=='1', 1, 0)

        pd_brand['num_times_mailed'] = np.where(pd_brand[f'{brand}_num_times_mailed'] > 0, pd_brand[f'{brand}_num_times_mailed'], -999)
