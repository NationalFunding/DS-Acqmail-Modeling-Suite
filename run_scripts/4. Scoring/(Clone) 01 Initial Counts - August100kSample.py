...
mailable_pop_tbl_spark = spark.sql(f"""SELECT * FROM nf_dev_workarea.model_data_input_v7""") 

# Take a reproducible random sample
sample_df = (
    mailable_pop_tbl_spark
...
run_dt = datetime.now().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
first_of_this_month = run_dt.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
mail_dt = first_of_this_month + relativedelta(months=2)
load_dt = first_of_this_month + relativedelta(months=-1)
load_month = load_dt
load_dt_prior = first_of_this_month + relativedelta(months=-2)
load_month_prior = load_dt_prior
mail_date_str = mail_dt.strftime('%Y-%m-%d')
run_year_month = run_dt.strftime('%Y-%m')
tib_suppression_date = mail_dt + relativedelta(months=-12)
# Getting load month quarter
rem = load_dt.month % 3
months_back = rem if rem != 0 else 0
closest_qtr_dt = load_dt - relativedelta(months=months_back)
lyr_qtr = closest_qtr_dt.year
lmon_qtr = closest_qtr_dt.month


# COMMAND ----------


mail_date_col = F.to_date(F.lit(mail_dt), 'yyyy-MM-dd')

sample_df = sample_df.withColumn("load_month", F.lit(load_month))
sample_df = sample_df.withColumn("load_month_prior", F.lit(load_month_prior))
sample_df = sample_df.withColumn("mail_date", F.lit(mail_dt))
...
sample_df = sample_df.withColumn(
    ...
    F.round(
        F.greatest(
            F.months_between(mail_date_col, F.col("nf_first_mail_date")),
            F.months_between(mail_date_col, F.col("qb_first_mail_date")),
        ),
        2
    )
)
...
sample_df.write.format("delta").mode("overwrite").saveAsTable(
    "nf_dev_workarea.v7_mailable_pop_scored_temp"
)

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from nf_dev_workarea.v7_mailable_pop_scored_temp
# MAGIC limit 10
        ...
        if brand == 'nf':
            ...
            for flg_ppp_ in ['N','Y']:
                ...
                for change_ in [0, 1]:
                    ...
                    session.log.info(f"################################################################")
                    model_input = spark.sql(f"""SELECT * FROM nf_dev_workarea.v7_mailable_pop_scored_temp
...
with open(STAGING_FOLDER + f'/nfqb_v7_august_TEMP_100k.pkl', 'wb') as handle:
...
file_path = STAGING_FOLDER + f'/nfqb_v7_august_TEMP_100k.pkl'
